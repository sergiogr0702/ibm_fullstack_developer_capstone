...
const SearchCars = () => {
    ...
    let cars_url = `/djangoapp/get_inventory/${id}`;
    ...
    const populateMakesAndModels = (cars)=>{
        ...
        let uniqueModels = [];
        cars.map((car)=>{
            allMakes.push(car.make);
            allModels.push(car.model);
        });
        uniqueMakes = new Set(allMakes);
        uniqueModels = new Set(allModels);
        setMakes(Array.from(uniqueMakes));
    ...
    const SearchCarsByMileage = async ()=>{
        ...
        if (mileage !== "all") {
            cars_url = cars_url + "?mileage="+mileage;
        }
    ...
    return (
        <div>
          <Header />
          <h1 style={{ marginBottom: '20px', marginTop: '20px'}}>Cars available at {dealer.full_name}</h1>
          <div>
          ...
          <button style={{marginLeft: '10px', paddingLeft: '10px', borderRadius :'10px'}} onClick={reset}>Reset</button>
          </div>
